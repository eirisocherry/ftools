...
rem Usage: simply drag and drop videos/folders onto the .bat file and wait till converting is finished
...
rem Choose a framerate (use only 0/75/120/125/250/500/1000 fps to avoid problems)
rem 0: don't change
rem 120: set to 120 fps
rem 1000: set to 1000 fps
rem etc...
set fps=120
...
set "lite=ffmpeg -hide_banner -loglevel error -stats %%framerate_dont_maintain_length%% -i "%%input_render%%" %%framerate_maintain_length%% %%resolution_ffmpeg%% -c:v libx264 -crf 20 -preset fast -tune fastdecode -pix_fmt yuv420p -color_primaries bt709 -color_trc bt709 -colorspace bt709 -level:v 5.2 -x264-params keyint=12:min-keyint=1:ref=1:bframes=0:qcomp=0.8:aq-strength=0.5:dct-decimate=0:fast-pskip=0:deblock=-2,-2 -c:a aac -b:a 512k %%disable_audio%% "%%output_render%%.mp4""
...
if exist "%~1\*" (
	...
	if "%~x1"==".mp4" (set "input_type=video")
	if "%~x1"==".m4v" (set "input_type=video")
	if "%~x1"==".avi" (set "input_type=video")
	if "%~x1"==".mkv" (set "input_type=video")
	if "%~x1"==".mov" (set "input_type=video")
	if "%~x1"==".wmv" (set "input_type=video")
	if "%~x1"==".webm" (set "input_type=video")
)
exit /b
...
if "%input_type%"=="" (
	...
	echo Supported formats: .mp4 .m4v .avi .mkv .mov .wmv .webm
	echo.
	exit /b
)
...
:video
set "input_video=%~1"
set "output_render=%~dp1converted_%preset%\%~n1"
...
mkdir "%~dp1converted_%preset%" >nul 2>&1
...
for /r "%input_folder%" %%A in (*.mp4 *.m4v *.avi *.mkv *.mov *.wmv *.webm) do (
	set "input_video=%%A"
	set "output_render=%output_folder%\%%~nA"
	rem Get current framerate
	set "current_framerate=ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate -of default=noprint_wrappers=1:nokey=1 "%%input_video%%""
	for /f "tokens=*" %%a in ('call %%current_framerate%%') do set current_framerate=%%a
	rem Get current resolution
...
if "%current_audio_frequency%"=="" (
	...
	set "disable_audio_info=Disabled, because an audio is missing"
) else (
	if "%audio_render%"=="1" (
		set "disable_audio="
